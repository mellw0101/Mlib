...
enum element_internal_state { MOUSE_INSIDE = 1, HIGHLIGHT_ON_HOVER };

/* ------------------ */
/*-<< element_text >>-*/
/* ------------------ */

void element_text::query_text_texture(float x, float y, SDL_Texture *t) noexcept {
  (x != -1) ? this->x = x : 0;
  (y != -1) ? this->y = y : 0;
  t ? texture = t : nullptr;
...
void element::init(void) noexcept {
}

void element::draw_borders(void) noexcept {
  if (border_size <= 0 || border_size >= rect.w || border_size >= rect.h) {
    return;
  }
  flags.is_set<HIGHLIGHT_ON_HOVER>() && flags.is_set<MOUSE_INSIDE>() ? set_ren_color(*renptr, &highlight_border_color)
...
void element::draw(void) noexcept {
  (flags.is_set<HIGHLIGHT_ON_HOVER>() && flags.is_set<MOUSE_INSIDE>()) ? set_ren_color(*renptr, &highlight_color)
...
void element::add_text_data(float x, float y, SDL_Texture *texture) noexcept {
  if (texture == nullptr) {
...
void element::add_text_data(TTF_Font *font, float x, float y, SDL_Color fg, SDL_Color bg, const char *str) noexcept {
  if (str == nullptr) {
    logE("str == nullptr.");
    return;
  }
...
void element::animate(float end_x, float end_y, float end_w, float end_h, int duration_ms) noexcept {
...
void element::set_borders(float size, SDL_Color color) noexcept {
  border_size  = size;
  border_color = color;
}

void element::set_highlight_on_hover(SDL_Color color, SDL_Color border_color) noexcept {
...
void element::set_text_highlight_on_hover(SDL_Color fg, SDL_Color bg) noexcept {
...
/*-<< sdl_button_element >>-*/
void element::_add_callback(void) noexcept {
...
void element::_gravity(void) noexcept {
