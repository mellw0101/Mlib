...
namespace Mlib::Sdl2
{
    ...
    void
    Core::create_object(const Object2D &object)
    {
...
{
    double
    screen_dpi(int sc_pix_w, int sc_pix_h, double sc_inch_w, double sc_inch_h)
    {
    ...
    SDL_Surface *
    create_sdl_surface_from_bitfield(MVector<MVector<int>> data)
    {
        int *pixels = AMALLOC_ARRAY(pixels, (data.size() * data.size()));
        ...
        SDL_Surface *surface = SDL_CreateRGBSurfaceFrom(pixels, (int)data.size(), (int)data.size(),
                                                        32, (int)data.size() * 4, 0x00FF0000,
    ...
    void
    convert_sdl_rect_to_floats(const SDL_Rect *rect, float *floats)
    {
    ...
    void
    sdl_simd_update_rect(SDL_FRect *frect, const SDL_FRect *end_frect, const float *step_vals)
    {
    ...
    void
    sdl_simd_update_rect_avx(SDL_FRect *frect, const SDL_FRect *end_frect, const float *step_vals)
    {
        /* Load the current rectangle and the end rectangle into AVX registers */
        __m256 rect_vals = _mm256_load_ps((const float *)frect);
        __m256 end_vals  = _mm256_load_ps((const float *)end_frect);
        /* Load step values for x, y, w, h */
        __m256 step_vals_avx = _mm256_load_ps(step_vals);
        /* Step comparisons: step < 0.0f */
        ...
        /* Store the updated values back into the rectangle */
        _mm256_store_ps(reinterpret_cast<float *>(frect), updated_rect);
    }
}
...
{
    int
    set_render_draw_color(SDL_Renderer *ren, unsigned char r, unsigned char g, unsigned char b,
...
{
    /*-<< sdl_event_handler >>-*/
    sdl_event_handler *sdl_event_handler::_instance = nullptr;
    void
    sdl_event_handler::_destroy(void) noexcept
    {
        delete _instance;
    }

    sdl_event_handler *
    sdl_event_handler::instance(void) noexcept
    {
        if (!_instance)
        {
            _instance = new (nothrow) sdl_event_handler();
            if (!_instance)
            {
    ...
    void
    sdl_event_handler::handle_event(void) noexcept
    {
    ...
    /*-<< sdl_element_text >>-*/
    void
    sdl_element_text::query_text_texture(int x, int y, SDL_Texture *t) noexcept
    {
        (x != -1) ? this->x = x : 0;
        (y != -1) ? this->y = y : 0;
    ...
    /*-<< sdl_element >>-*/
    void
    sdl_element::draw_borders(void) noexcept
    {
    ...
    void
    sdl_element::draw(void) noexcept
    {
    ...
    void
    sdl_element::add_text_data(int x, int y, SDL_Texture *texture) noexcept
    {
        if (texture == nullptr)
        {
    ...
    void
    sdl_element::animate(int end_x, int end_y, int end_w, int end_h, int duration_ms) noexcept
    {
        if (!flags.is_set(ELEMENT_IN_ANIMATION))
        {
            logI("duration_ms: %i", duration_ms);
            double steps = duration_ms / FRAME_DELAY(SDL_APP->framerate());
            logI("steps: %lf", steps);
    ...
    void
    sdl_element::set_borders(int size, SDL_Color color) noexcept
    {
        border_size  = size;
        border_color = color;
    }
    ...
    void
    sdl_element::check_flags(void) noexcept
    {
        if (flags.is_set(ELEMENT_ALIGN_WIDTH))
        {
            SDL_EVENT_HANDLER->event_action(SDL_WINDOWEVENT, [this](SDL_Event ev) {
    ...
    sdl_element::sdl_element_t(void) noexcept
    {}

    /*-<< sdl_button_element >>-*/
    void
    sdl_button_element::_add_callback(void) noexcept
    {
        SDL_EVENT_HANDLER->event_action(SDL_MOUSEBUTTONDOWN, [this](SDL_Event ev) {
    ...
    /*-<< Constructor >>-*/
    sdl_app::sdl_app_t(void) noexcept : mem_pool(100 * MB)
    {
        if (SDL_Init(SDL_INIT_VIDEO) == -1)
        {
            logE("Failed to init sdl, SDL_ERROR: %s.", SDL_GetError());
        ...
        SDL_EVENT_HANDLER->event_action(SDL_WINDOWEVENT, [this](SDL_Event ev) {
            ...
            switch (e.event)
            {
                case SDL_WINDOWEVENT_SIZE_CHANGED :
                {
                    ...
                    {
                        if (elem->flags.is_set(ELEMENT_ALIGN_WIDTH))
                        {
                            elem->rect.w = _width;
                        }
                    }
        ...
        SDL_EVENT_HANDLER->event_action(SDL_QUIT, [this](SDL_Event ev) {
            quit();
        });
    ...
    /*-<< Destructor >>-*/
    void
    sdl_app::_destroy(void) noexcept
    {
    ...
    void
    sdl_app::_cleanup(void) noexcept
    {
    ...
    void
    sdl_app::_perform_amimation(void) noexcept
    {
        for (unsigned int i = 0; i < _anim_data.size(); i++)
        {
            ...
            if (steps <= 0.00)
            {
                _anim_data[i].first->flags.unset(ELEMENT_IN_ANIMATION);
    ...
    void
    sdl_app::_frame_start(void) noexcept
    {
        _frame_start_time = high_resolution_clock::now();
    }

    void
    sdl_app::_frame_end(void) noexcept
    {
        time_point<high_resolution_clock> end = high_resolution_clock::now();
        duration<double, milli>           dur = end - _frame_start_time;
        auto delay_duration = duration<double, milli>(FRAME_DELAY(_framerate) - dur.count());
        sleep_for(delay_duration);
    }

    /* This is the only way to access the app. */
    sdl_app *
    sdl_app::instance(void) noexcept
    {
        if (_instance == nullptr)
        {
    ...
    void
    sdl_app::set_main_loop(function<void()> loop) noexcept
    {
        if (loop != nullptr)
        {
            _main_loop = loop;
    ...
    /* Add a element to the app.  Remember to add a 0 at the end of the flags array. */
    sdl_element *
    sdl_app::new_element(SDL_Color color, SDL_Rect rect,
                         const init_list<unsigned int> &flags) noexcept
    {

        sdl_element *element = mem_pool.alloc<sdl_element>();
        if (element == nullptr)
        {
        ...
        element->rect   = rect;
        element->renptr = &_ren;
        element->flags  = bit_flag_t<8>(flags);
        if (element->flags.is_set(ELEMENT_ALIGN_WIDTH))
        {
            /* SDL_EVENT_HANDLER->event_action(SDL_WINDOWEVENT, [&element](SDL_Event ev) {
                SDL_WindowEvent e = ev.window;
                switch (e.event)
                {
                    case SDL_WINDOWEVENT_SIZE_CHANGED :
    ...
    sdl_button_element *
    sdl_app::new_button_element(sdl_element element) noexcept
    {
    ...
    TTF_Font *
    sdl_app::retrieve_new_font(const int size, const char *file) noexcept
    {
    ...
    SDL_Texture *
    sdl_app::make_text_texture(TTF_Font *font, const char *text, SDL_Color fg,
    ...
    SDL_Texture *
    sdl_app::make_destroy_text_texture(SDL_Texture *old_texture, TTF_Font *font, const char *text,
    ...
    void
    sdl_app::render_text(int x, int y, SDL_Texture *texture) noexcept
    {
        if (texture == nullptr)
        {
            logE("texture is nullptr.");
    ...
    void
    sdl_app::move_element(unsigned int id, int x, int y)
    {
    ...
    int
    sdl_app::width(void) const noexcept
    {
        return _width;
    }

    int
    sdl_app::height(void) const noexcept
    {
        return _height;
    }
    ...
    void
    sdl_app::set_size(const int w, const int h) noexcept
    {
    ...
    void
    sdl_app::set_position(const int x, const int y) noexcept
    {
        SDL_SetWindowPosition(_win, x, y);
    }

    void
    sdl_app::set_title(const char *str) noexcept
    {
        SDL_SetWindowTitle(_win, str);
    }

    void
    sdl_app::set_min_size(const int w, const int h) noexcept
    {
        SDL_SetWindowMinimumSize(_win, w, h);
    }

    sdl_element *
    sdl_app::element_from_id(unsigned int id) noexcept
    {
    ...
    void
    sdl_app::exit_clean(const int status) noexcept
    {
        _cleanup();
        exit(status);
    }
    ...
    int
    sdl_app::set_framerate(const int new_framerate) noexcept
    {
    ...
    int
    sdl_app::framerate(void) const noexcept
    {
        return _framerate;
    }

    void
    sdl_app::animate_element(sdl_element *element, sdl_element_animation_data data) noexcept
    {
        if (!element->flags.is_set(ELEMENT_IN_ANIMATION))
        {
            element->flags.set(ELEMENT_IN_ANIMATION);
