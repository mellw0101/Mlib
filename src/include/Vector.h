...
public:
    ...
    __inline__ void __MVector_attr
    reorder_from(Uint newstart) noexcept
    {
        if (newstart < len)
        {
    ...
    __inline__ void __MVector_attr
    erase_at(Uint index) noexcept(std::is_nothrow_destructible<T>::value)
    {
        if (index < len)
        {
            data[index].~T();
    ...
    __inline__ void __MVector_attr
    clear(void) noexcept(std::is_nothrow_destructible<T>::value)
    {
    ...
    __inline__ Uint __warn_unused __MVector_attr
    size(void) noexcept
    {
        return len;
    }

    __inline__ const Uint __warn_unused __MVector_const_attr
    size(void) const noexcept
    {
        return len;
    }

    __inline__ bool __warn_unused __MVector_attr
    empty(void) noexcept
    {
        return (len == 0);
    }

    __inline__ const bool __warn_unused __MVector_const_attr
    ...
    __inline__ void __MVector_attr
    resize(const Uint newlen) noexcept
    {
        if (newlen < len)
        {
            len = newlen;
    ...
    __inline__ T & __MVector_attr
    back(void) noexcept
    {
        return (len > 0) ? *(data + (len - 1)) : *data;
    }

    __inline__ const T & __MVector_const_attr
    back(void) const noexcept
    {
        return (len > 0) ? *(data + (len - 1)) : *data;
    }

    __inline__ T *__MVector_const_attr
    ...
    MVector(initializer_list<T> list)
    noexcept(std::is_nothrow_default_constructible<T>::value)
        : len(0), cap(list.size() * 2)
    {
        data = (T *)calloc(cap, sizeof(T));
    ...
    /* Operator. */
    __inline__ T &__MVector_attr
    operator[](const Uint index) noexcept
    {
        return data[index];
    }

    __inline__ const T &__MVector_const_attr
    operator[](const Uint index) const noexcept
    {
        return data[index];
    }
};
