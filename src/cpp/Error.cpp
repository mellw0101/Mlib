...
namespace Mlib::Error
{
    ...
    fatal_error(const char *from_function, const char *when_calling, const char *format, ...)
    {
        ...
        fwritef(STDERR_FILENO, "%s: from_function: [%s], when_calling: [%s], ERROR: [%s], %s.\n", __func__,
                from_function, when_calling, strerror(errno), buf);
        exit(1);
    }

    void
    non_fatal_error(bool include_errno_str, const char *from_function, const char *when_calling, const char *format,
                    ...)
    {
    ...
    void
    program_error(const char *prefix, const char *from_func, unsigned at_line, const char *when_calling,
                  const char *format, ...)
    {
        static char buf[4096];
        va_list     ap;
