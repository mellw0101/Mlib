...
namespace Mlib::Sys {
  ...
  Prompt::operator std::string(void) const {
    return ss.str();
  }
  ...
  Ulong retriveSysLogicCores(void) {
    Ulong sys_cores = sysconf(_SC_NPROCESSORS_ONLN);
    if (sys_cores == -1) {
  ...
  Ulong read_msr_value_to_watts(MSRReader *msr_reader, Uint msr_address) {
    Ulong raw_value = msr_reader->read(msr_address);
...
void format_time(Ulong ms, char *buffer, Ulong buffer_size) {
  ...
  if (!tm_info) {
    snprintf(buffer, buffer_size, "Error formating time");
    return;
  }
  strftime(buffer, buffer_size, "%Y-%m-%d %H:%M:%S", tm_info);
  const Ulong buffer_len = strlen(buffer);
  snprintf((buffer + buffer_len), (buffer_size - buffer_len), ".%03lu", (Ulong)(ms % 1000));
}